...
router.post('/visit-entries', auth.authenticate, auth.requireRole('attraction'), async (req, res, next) => {
  ...
  try {
    ...
    const [attractions] = await connection.execute(
      'SELECT id FROM tourist_attractions WHERE user_id = ? AND deleted_at IS NULL AND status = \'approved\'',
      [req.user.id]
    );
    ...
    await connection.execute(
      ...
      [
        ...
        femaleCountInt,
        resolvedCountry,
        !isForeign ? (province || null) : null,
        !isForeign ? (cityMunicipality || null) : null,
      ]
    );
